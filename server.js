...
// 1. Security Headers (Helmet) - STRICT CSP (no inline scripts allowed)
app.use(
  helmet({
    contentSecurityPolicy: {
      directives: {
        ...
        scriptSrc: ["'self'"],
        scriptSrcAttr: [], // No inline event handlers allowed
...
const UserSchema = new mongoose.Schema({
  ...
  email: {
    ...
    trim: true,
    lowercase: true,
    validate: { validator: function (v) { return !v || validator.isEmail(v); }, message: "Invalid email format" },
  },
  phone: { type: String, trim: true, maxlength: [20, "Phone number too long"] },
...
const PartSchema = new mongoose.Schema({
  partNumber: { type: String, required: true, unique: true, trim: true, maxlength: [50, "Part number too long"] },
  description: { type: String, required: true, trim: true, maxlength: [500, "Description too long"] },
  category: {
    type: String,
    required: true,
    enum: ["Engine", "Transmission", "Brakes", "Electrical", "Body", "Tires", "Other"],
  },
  quantity: { type: Number, default: 0, min: [0, "Quantity cannot be negative"], max: [999999, "Quantity too high"] },
  minStock: { type: Number, default: 1, min: [0, "Min stock cannot be negative"] },
  location: { type: String, trim: true, maxlength: [100, "Location too long"] },
  cost: { type: Number, default: 0, min: [0, "Cost cannot be negative"] },
  supplier: { type: String, trim: true, maxlength: [100, "Supplier name too long"] },
  createdBy: { type: mongoose.Schema.Types.ObjectId, ref: "User" },
...
const TruckSchema = new mongoose.Schema({
  truckId: { type: String, required: true, unique: true, trim: true, uppercase: true, maxlength: [20, "Truck ID too long"] },
  name: { type: String, required: true, trim: true, maxlength: [100, "Name too long"] },
  type: { type: String, enum: ["truck", "trailer"], required: true },
  vin: { type: String, trim: true, uppercase: true, maxlength: [17, "VIN must be 17 characters"] },
  year: { type: Number, min: [1980, "Year too old"], max: [2030, "Year too far in future"] },
  make: { type: String, trim: true, maxlength: [50, "Make name too long"] },
  model: { type: String, trim: true, maxlength: [50, "Model name too long"] },
  notes: { type: String, trim: true, maxlength: [2000, "Notes too long"] },
...
const RepairSchema = new mongoose.Schema({
  ...
  truckId: { type: String, required: true, trim: true, maxlength: [20, "Truck ID too long"] },
  truckName: { type: String, trim: true, maxlength: [100, "Truck name too long"] },
  issue: { type: String, required: true, trim: true, maxlength: [2000, "Issue description too long"] },
  partsUsed: [{
  ...
  mechanic: { type: String, trim: true, maxlength: [100, "Mechanic name too long"] },
...
const updateTruckCosts = async (truckId) => {
  try {
    ...
    console.error("Update truck costs error:", e);
  }
};
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (typeof username !== "string" || typeof password !== "string") return res.status(400).json({ error: "Invalid input format" });
...
app.post("/api/users", authenticate, requireRole(["admin"]), async (req, res) => {
  try {
    ...
    if (email && !validator.isEmail(email)) return res.status(400).json({ error: "Invalid email format" });
...
app.patch("/api/users/:id", authenticate, requireRole(["admin"]), async (req, res) => {
  try {
    ...
    if (req.params.id === req.user.userId && isActive === false) return res.status(400).json({ error: "Cannot disable your own account" });
...
app.get("/api/vehicles/:truckId/stats", authenticate, async (req, res) => {
  try {
    ...
    if (!truckId || !/^[A-Z0-9-]+$/i.test(truckId)) return res.status(400).json({ error: "Invalid truck ID format" });

    const truck = await Truck.findOne({ truckId: truckId.toUpperCase() });
    if (!truck) return res.status(404).json({ error: "Vehicle not found" });

    const repairs = await Repair.find({ truckId: truckId.toUpperCase() }).sort({ date: -1 });
...
app.post("/api/trucks", authenticate, requireRole(["admin"]), async (req, res) => {
  try {
    ...
    await truck.save();
    await logActivity(req.user.userId, req.user.username, "CREATE_TRUCK", "Truck", truck.truckId, req.body, req);
...
app.get("/api/repairs", authenticate, async (req, res) => {
  try {
    let query = {};
    if (req.user.role === "mechanic") query.mechanicId = req.user.userId;
...
app.post("/api/repairs", authenticate, requireRole(["admin", "mechanic"]), async (req, res) => {
  try {
    ...
    await logActivity(req.user.userId, req.user.username, "CREATE_REPAIR", "Repair", repair._id, {
      truckId, truckName: truck.name, totalCost, partsCount: processedParts.length,
    }, req);
...
app.get("/api/export", authenticate, requireRole(["admin"]), async (req, res) => {
  try {
    const [parts, trucks, repairs, users] = await Promise.all([
      Part.find(), Truck.find(), Repair.find(), User.find().select("-password"),
    ]);
...
const createDefaultAdmin = async () => {
  try {
    ...
    if (count === 0) {
      ...
      console.log("IMPORTANT: Change default password immediately after first login!");
